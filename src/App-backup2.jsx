...
function App() {
  ...
  function getDailyTakingsSummary() {
    const base = { totalRevenue: 0, cardTotal: 0, cashTotal: 0, bankTransferTotal: 0, otherTotal: 0, totalMinutes: 0, paymentCount: 0, productRevenue: 0 }
  ...
  async function createShopTestBookingFromModal() {
    const signedInStaff = getCurrentStaffUser()

    if (!signedInStaff) {
      alert('Please sign in as staff before creating a shop test.')
  ...
  async function createBookingFromModal() {
    ...
    if (!signedInStaff) {
      alert('Please sign in as staff before creating a booking.')
      setStaffSelectorOpen(true)
      return
    }
  ...
  async function createStaffFreeBookingFromModal(member) {
    ...
    if (!signedInStaff) {
      alert('Please sign in as staff before creating a staff free booking.')
      setStaffSelectorOpen(true)
      return
    }
  ...
  async function saveEditedBooking() {
    ...
    if (!signedInStaff) {
      alert('Please sign in as staff before editing a booking.')
      setStaffSelectorOpen(true)
      return
    }
  ...
  async function startSession(booking) {
    ...
    if (!signedInStaff) {
      alert('Please sign in as staff before starting a session.')
      setStaffSelectorOpen(true)
      return
    }
  ...
  async function customerStartedBed(booking) {
    ...
    if (!signedInStaff) {
      alert('Please sign in as staff before confirming the customer has started the bed.')
      setStaffSelectorOpen(true)
      return
    }
        ...
        {productCart.length === 0 ? (
          ...
          productCart.map((item) => (
            <div key={item.product_id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px', gap: '8px', alignItems: 'center', borderBottom: '1px solid #333', padding: '8px 0' }}>
              <span>{item.product_name}<br /><small>£{Number(item.price || 0).toFixed(2)}</small></span>
  ...
  function renderProductPicker() {
    return (
      <div style={{ marginTop: '12px' }}>
        <button type="button" onClick={() => setShowProductPicker(true)}>+ Add Products</button>
        {productCart.length > 0 && (
          <>
            {renderProductCart()}
                ...
                getActiveProducts().map((product) => (
                  <div key={product.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'center', background: '#111', padding: '10px', borderRadius: '12px', marginBottom: '8px' }}>
                    <div>
                      ...
                      <small>{formatStatus(product.category)} — £{Number(product.price || 0).toFixed(2)} {Number(product.stock_quantity || 0) > 0 ? `— Stock ${product.stock_quantity}` : ''}</small>
                    </div>
  ...
  function renderProductsManagementPanel() {
    ...
    return renderCollapsibleSection(
      ...
      <>
        ...
        <button onClick={() => setShowStandalonePOS(true)} style={{ marginBottom: '15px' }}>Products / POS</button>
          ...
          >
            ...
            {products.map((product) => (
              <option key={product.id} value={product.id}>
                {product.name} — {formatStatus(product.category)} — £{Number(product.price || 0).toFixed(2)} — Stock {product.stock_quantity || 0} — {product.is_active === false ? 'Inactive' : 'Active'}
              </option>
            ))}
          </select>
          ...
          {selectedProduct && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '10px', alignItems: 'center', marginTop: '12px', padding: '12px', background: '#111', borderRadius: '12px' }}>
              <div>
                ...
                <span>{formatStatus(selectedProduct.category)} — £{Number(selectedProduct.price || 0).toFixed(2)} — Stock {selectedProduct.stock_quantity || 0}</span><br />
        ...
        {showStandalonePOS && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#1e1e1e', padding: '24px', borderRadius: '18px', width: '560px', maxWidth: '92%', maxHeight: '85vh', overflowY: 'auto' }}>
              ...
              {getActiveProducts().map((product) => (
                <div key={product.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'center', background: '#111', padding: '10px', borderRadius: '12px', marginBottom: '8px' }}>
                  <div><strong>{product.name}</strong><br /><small>£{Number(product.price || 0).toFixed(2)}</small></div>
            ...
            {!modalBooking ? (
              <>
                ...
                <div style={{ background: '#0b0b0b', border: '1px solid #333', borderRadius: '12px', padding: '12px', marginBottom: '12px', textAlign: 'center' }}>
                  ...
                  <button
                    ...
                    disabled={!getCurrentStaffUser()}
                    title={!getCurrentStaffUser() ? 'Staff must sign in before creating shop tests' : ''}
                  >
                ...
                <button
                  ...
                  disabled={!getCurrentStaffUser()}
                  title={!getCurrentStaffUser() ? 'Staff must sign in before creating bookings' : ''}
                >
                  Create Booking
                </button>
              ...
              <>
                ...
                <button
                  ...
                  disabled={!getCurrentStaffUser()}
                  title={!getCurrentStaffUser() ? 'Staff must sign in before editing bookings' : ''}
                >
                  Save Changes
                </button>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '20px' }}>
                  {!modalBooking.booking_start && !['completed', 'no_show', 'force_stopped'].includes(String(modalBooking.status || '').toLowerCase()) && (
                    <button
                      ...
                      title={!getCurrentStaffUser() ? 'Staff must sign in before starting sessions' : ''}
                    >
