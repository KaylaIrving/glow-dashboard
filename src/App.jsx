...
function App() {
  ...
  const [commissionRuleScope, setCommissionRuleScope] = useState('product')
  const [commissionRuleMatch, setCommissionRuleMatch] = useState('')
  const [commissionRuleType, setCommissionRuleType] = useState('percentage')
...
function formatMoney(value) {
  ...
  function exportStaffCalendarPdf() {
    if (!requireStaffSignIn()) return
    if (!requireManagerAccess('Manager PIN required to export Staff Calendar:')) return
    ...
    const weekLabel = `${new Date(`${weekDates[0]}T00:00:00`).toLocaleDateString('en-GB')} - ${new Date(`${weekDates[6]}T00:00:00`).toLocaleDateString('en-GB')}`
    const rows = visibleStaff.map((member) => {
      const cells = weekDates.map((date) => {
        const entries = filteredEntries
          .filter((entry) => String(entry.staff_id) === String(member.id) && entry.schedule_date === date)
          .map((entry) => `<div class="entry"><strong>${escapeHtml(formatStatus(entry.schedule_type))}</strong><br>${escapeHtml(formatStaffScheduleTime(entry.start_time))} - ${escapeHtml(formatStaffScheduleTime(entry.end_time))}<br>${escapeHtml(getStaffScheduleApprovalStatus(entry))}${entry.notes ? `<br><small>${escapeHtml(entry.notes)}</small>` : ''}</div>`)
          .join('')
        return `<td>${entries || '<span class="empty">-</span>'}</td>`
      }).join('')
      return `<tr><th>${escapeHtml(member.name)}</th>${cells}</tr>`
    }).join('')
    const headerCells = weekDates.map((date) => `<th>${escapeHtml(new Date(`${date}T00:00:00`).toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short' }))}</th>`).join('')
    const printWindow = window.open('', '_blank')
    if (!printWindow) {
      alert('Popup blocked. Please allow popups to export the staff calendar PDF.')
      return
    }
    printWindow.document.write(`
      <html>
        <head>
          <title>Glow Staff Calendar ${escapeHtml(weekLabel)}</title>
          <style>
            body { font-family: Arial, sans-serif; color: #17120b; margin: 24px; }
            h1 { margin: 0 0 4px; color: #7a5e2d; }
            p { margin: 0 0 18px; color: #555; }
            table { width: 100%; border-collapse: collapse; font-size: 12px; }
            th, td { border: 1px solid #c9b98f; padding: 8px; vertical-align: top; }
            th { background: #f3ead8; color: #3b2a12; text-align: left; }
            .entry { background: #fff8e8; border: 1px solid #d9c38b; padding: 6px; margin-bottom: 5px; }
            .empty { color: #999; }
        ...
        <body>
          ...
          <h1>Glow Staff Calendar</h1>
          <p>${escapeHtml(weekLabel)} · ${staffScheduleFilterStaffId ? escapeHtml(visibleStaff[0]?.name || 'Selected staff') : 'All staff'}</p>
          <table>
            <thead><tr><th>Staff</th>${headerCells}</tr></thead>
            <tbody>${rows}</tbody>
          </table>
  ...
  async function getCommissionRules() {
    ...
    if (error) {
      setCommissionRules([])
      setCommissionRulesError(error.message || 'CommissionSettings table could not be loaded.')
      return
    }
  ...
  function clearCommissionRuleForm() {
    setCommissionRuleEditingId('')
    setCommissionRuleScope('product')
    setCommissionRuleMatch('')
    setCommissionRuleType('percentage')
    setCommissionRuleValue('')
    setCommissionRuleStaffId('')
    setCommissionRuleActive(true)
  }

  async function saveCommissionRule() {
    if (!requireStaffSignIn()) return
    if (!requireManagerAccess('Manager PIN required to save commission settings:')) return
    const selectedStaff = staff.find((member) => String(member.id) === String(commissionRuleStaffId))
    const payload = {
      rule_scope: commissionRuleScope,
      match_value: commissionRuleMatch.trim(),
      commission_type: commissionRuleType,
      commission_value: Number(commissionRuleValue || 0),
      is_active: commissionRuleActive,
      staff_id: selectedStaff?.id || null,
      staff_name: selectedStaff?.name || null
    }
    if (!payload.match_value) {
      alert('Enter what this commission rule applies to.')
      return
    }
    if (payload.commission_value < 0) {
      alert('Commission value cannot be negative.')
      return
    }

    const request = commissionRuleEditingId
      ? supabase.from('CommissionSettings').update(payload).eq('id', commissionRuleEditingId)
      : supabase.from('CommissionSettings').insert(payload)
    const { error } = await request
    if (error) {
      alert('Commission rule was not saved. Please check the CommissionSettings table.')
      showDataLoadWarning('Commission settings failed to save.', error)
      console.log(error)
      return
    }
    clearCommissionRuleForm()
    await getCommissionRules()
  }

  function editCommissionRule(rule) {
    setCommissionRuleEditingId(String(rule.id))
    setCommissionRuleScope(rule.rule_scope || 'product')
    setCommissionRuleMatch(rule.match_value || '')
    setCommissionRuleType(rule.commission_type || 'percentage')
    setCommissionRuleValue(rule.commission_value ?? '')
    setCommissionRuleStaffId(rule.staff_id ? String(rule.staff_id) : '')
    setCommissionRuleActive(rule.is_active !== false)
  }

  async function deleteCommissionRule(rule) {
    if (!requireStaffSignIn()) return
    if (!requireManagerAccess('Manager PIN required to delete commission settings:')) return
    if (!window.confirm('Delete this commission rule?')) return
    const { error } = await supabase.from('CommissionSettings').delete().eq('id', rule.id)
    if (error) {
      alert('Commission rule was not deleted.')
      showDataLoadWarning('Commission settings delete failed.', error)
      console.log(error)
      return
    }
    await getCommissionRules()
  ...
  function getCommissionRuleAmount({ scope, amount = 0, staffName = '', productName = '', category = '', serviceName = '' }) {
    ...
    return rule.commission_type === 'fixed' ? value : Number(amount || 0) * value / 100
  }
  ...
  async function generateManagerReports() {
    ...
    for (const sale of productSales) {
      ...
      const productRuleCommission = getCommissionRuleAmount({ scope: 'product', amount, staffName, productName: sale.product_name, category: sale.category })
      const categoryRuleCommission = getCommissionRuleAmount({ scope: 'category', amount, staffName, category: sale.category })
      row.estimated_commission += productRuleCommission || categoryRuleCommission || (amount * productCommissionRate / 100)
    }
    ...
    for (const payment of payments) {
      ...
      if (packageType === 'promo' || packageName.includes('promo')) {
        ...
        row.total_revenue += amount
        row.estimated_commission += getCommissionRuleAmount({ scope: 'spraytan', amount, staffName, serviceName: payment.package_name || payment.package_type }) || (amount * sprayTanCommissionRate / 100 + flatServiceCommission)
      } else {
  ...
  function getFilteredCustomerAndStaffOptions() {
    ...
    const scoreMatch = (values) => {
      const normalizedValues = values.filter(Boolean).map((value) => String(value).trim().toLowerCase())
      if (normalizedValues.some((value) => value.startsWith(query))) return 0
      if (normalizedValues.some((value) => value.includes(query))) return 1
      return null
    }
    const customerOptions = customers
      .map((customer) => {
        const fullName = [customer.first_name, customer.last_name].filter(Boolean).join(' ') || customer.name
        const score = scoreMatch([customer.first_name, customer.last_name, fullName, customer.name, customer.phone, customer.email])
        return score === null ? null : { kind: 'customer', id: customer.id, score, label: isShopTestCustomer(customer) ? 'Shop Test — Internal' : customer.name, record: customer }
      })
      .filter(Boolean)
    ...
    const staffOptions = staff
      .map((member) => {
        ...
        return score === null ? null : { kind: 'staff', id: member.id, score, label: `${member.name} - Staff`, record: member }
      })
      .filter(Boolean)
  ...
  function normalizeWixServiceRecord(service) {
  ...
  function getWixServiceDraftKey(service) {
  ...
  async function getWixServiceBookingMappings({ silent = false } = {}) {
    const { data, error } = await supabase
      ...
      .select('*')
      .order('wix_service_name', { ascending: true })

    if (error) {
      if (!silent) {
        console.error('Wix service booking map failed to load:', error)
        showToast('Wix service map could not be loaded.', 'error')
      }
      return wixServiceMappings
    }

    setWixServiceMappings(data || [])
    return data || []
  }

  async function reconcileRenamedWixServices(services, mappings = wixServiceMappings) {
  ...
  async function refreshWixServices() {
    if (!requireStaffSignIn()) return
    ...
    try {
      ...
      persistWixSyncDiagnostics(nextDiagnostics)
      setWixServiceMappings(refreshedMappings)
      showToast(`Refreshed ${services.length} active Wix service(s).`, 'success')
    } catch (error) {
      console.error('Refresh Wix services failed:', error)
      showToast(error.message || 'Wix services could not be refreshed.', 'error')
    } finally {
  ...
  function updateWixMappingDraft(service, field, value) {
    const key = getWixServiceDraftKey(service)
    setWixServiceMappingDrafts((current) => ({
      ...current,
      [key]: {
        ...(current[key] || {}),
        [field]: value
      ...
      <div style={panelStyle}>
        ...
        <div style={formGridStyle}>
          ...
          <select value={commissionRuleStaffId} onChange={(e) => setCommissionRuleStaffId(e.target.value)} style={{ padding: '10px' }}>
            <option value="">All staff</option>
            {staff.filter((member) => member.is_active !== false).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}
          </select>
        ...
        <div style={{ maxHeight: '260px', overflowY: 'auto', border: '1px solid #333', borderRadius: '10px' }}>
          {commissionRules.length === 0 ? (
            <p style={{ color: '#aaa', padding: '10px', margin: 0 }}>No commission rules saved yet.</p>
            ...
            <div key={rule.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '8px', alignItems: 'center', padding: '10px', borderBottom: '1px solid #222' }}>
              ...
              <button onClick={() => deleteCommissionRule(rule)}>Delete</button>
            </div>
          ))}
        </div>
      </div>
  ...
  function getDuplicateCustomerMatches() {
    ...
    const simpleName = (customer) => nameKey(customer).replace(/[^a-z0-9]/g, '')
    const matches = []
    const seen = new Set()
    const addMatch = (customerA, customerB, reason) => {
      if (!customerA?.id || !customerB?.id || customerA.id === customerB.id) return
      const key = [customerA.id, customerB.id].sort().join('-')
      const reasonKey = `${key}-${reason}`
      if (seen.has(reasonKey)) return
      seen.add(reasonKey)
      matches.push({ id: reasonKey, customerA, customerB, reason })
    }

    for (let index = 0; index < customers.length; index += 1) {
      for (let nextIndex = index + 1; nextIndex < customers.length; nextIndex += 1) {
        const customerA = customers[index]
        const customerB = customers[nextIndex]
        const phoneA = clean(customerA.phone).replace(/\s+/g, '')
        const phoneB = clean(customerB.phone).replace(/\s+/g, '')
        const emailA = clean(customerA.email)
        const emailB = clean(customerB.email)
        const fullNameA = nameKey(customerA)
        const fullNameB = nameKey(customerB)
        const postcodeA = clean(customerA.postcode).replace(/\s+/g, '')
        const postcodeB = clean(customerB.postcode).replace(/\s+/g, '')
        const compactNameA = simpleName(customerA)
        const compactNameB = simpleName(customerB)

        if (phoneA && phoneA === phoneB) addMatch(customerA, customerB, 'Same mobile number')
        if (emailA && emailA === emailB) addMatch(customerA, customerB, 'Same email address')
        if (fullNameA && fullNameA === fullNameB) addMatch(customerA, customerB, 'Same first and last name')
        if (postcodeA && postcodeA === postcodeB && compactNameA && compactNameB && (compactNameA.includes(compactNameB) || compactNameB.includes(compactNameA) || compactNameA.slice(0, 4) === compactNameB.slice(0, 4))) {
          addMatch(customerA, customerB, 'Similar name + same postcode')
        }
      }
    }
    return matches
  ...
  function renderDuplicateCustomersReportPanel() {
    if (!showManagerView) return null
    const matches = getDuplicateCustomerMatches()
    const cellStyle = { padding: '9px', borderBottom: '1px solid #222', verticalAlign: 'top' }

    return renderCollapsibleSection(
      'Duplicate Customers Report',
      collapseDuplicateCustomers,
      setCollapseDuplicateCustomers,
      <div style={{ background: '#0b0b0b', border: '1px solid #333', borderRadius: '14px', padding: '14px' }}>
        <p style={{ color: '#aaa', marginTop: 0 }}>Customer Data Health checks possible duplicate customer records. Merge is marked as a future feature for safety.</p>
        <div style={{ overflowX: 'auto', border: '1px solid #333', borderRadius: '10px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '760px' }}>
            <thead>
              <tr>
                <th style={cellStyle}>Customer A</th>
                <th style={cellStyle}>Customer B</th>
                <th style={cellStyle}>Reason for match</th>
                <th style={cellStyle}>Action</th>
              </tr>
            </thead>
            <tbody>
              {matches.length === 0 ? (
                <tr><td style={cellStyle} colSpan="4">No possible duplicates found.</td></tr>
              ) : matches.map((match) => (
                <tr key={match.id}>
                  <td style={cellStyle}><strong>{match.customerA.name || 'Unnamed'}</strong><br />{match.customerA.phone || 'No phone'}<br />{match.customerA.email || 'No email'}<br />{match.customerA.postcode || 'No postcode'}</td>
                  <td style={cellStyle}><strong>{match.customerB.name || 'Unnamed'}</strong><br />{match.customerB.phone || 'No phone'}<br />{match.customerB.email || 'No email'}<br />{match.customerB.postcode || 'No postcode'}</td>
                  <td style={cellStyle}>{match.reason}</td>
                  <td style={cellStyle}><button type="button" onClick={() => alert('Merge Customers is a future feature. Review these records manually for now.')}>Merge Customers</button></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  function getWixServicesForMapping() {
    ...
    const diagnosticServices = Array.isArray(diagnostics.services) ? diagnostics.services.map(normalizeWixServiceRecord).filter((service) => service.is_active) : []
    const failedServices = Array.isArray(diagnostics.errors)
      ? diagnostics.errors
        .filter((error) => String(error.message || '').toLowerCase().includes('needs mapping'))
        .map((error) => normalizeWixServiceRecord({ wix_service_id: error.serviceId, wix_service_name: error.serviceName }))
        .filter((service) => service.wix_service_id || service.wix_service_name)
      : []
    const mappedServices = wixServiceMappings
      .filter((mapping) => mapping.is_active !== false)
      .map((mapping) => normalizeWixServiceRecord(mapping))
    const serviceMap = new Map()
    ;[...diagnosticServices, ...failedServices, ...mappedServices].filter(Boolean).forEach((service) => {
      const normalized = normalizeWixServiceRecord(service)
      const key = normalized.wix_service_id || normalizeWixServiceKey(normalized.wix_service_name)
      const archived = getAnyWixMappingForService(normalized)?.is_active === false
      if (!key || archived) return
      if (!serviceMap.has(key)) serviceMap.set(key, normalized)
    })
    return [...serviceMap.values()].sort((a, b) => a.wix_service_name.localeCompare(b.wix_service_name))
  }
  ...
  function renderWixBookingSyncPanel() {
    ...
    const activeWixServices = (Array.isArray(diagnostics.services) ? diagnostics.services : []).map(normalizeWixServiceRecord).filter((service) => service.is_active)
    const activeWixServiceIds = new Set(activeWixServices.map((service) => service.wix_service_id).filter(Boolean))
    const liveServicesMapped = activeWixServices.filter((service) => {
      const mapping = getAnyWixMappingForService(service)
      return mapping && mapping.is_active !== false
    }).length
    const activeUnmappedServices = activeWixServices.filter((service) => !getAnyWixMappingForService(service))
        ...
        <div style={{ background: '#111', border: '1px solid rgba(212,168,83,0.28)', borderRadius: '10px', padding: '12px', marginBottom: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '10px' }}>
            <div>
              ...
              <p style={{ color: '#aaa', margin: 0 }}>Manager &gt; Integrations &gt; Wix &gt; Service Booking Map</p>
            </div>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button type="button" onClick={refreshWixServices} disabled={wixSyncRunning}>Refresh Wix Services</button>
              <button type="button" onClick={() => runWixBookingSync()} disabled={wixSyncRunning}>
                {wixSyncRunning ? 'Retrying...' : 'Retry Failed Bookings'}
              </button>
            </div>
          </div>

          {wixServicesForMapping.length === 0 ? (
            <p style={{ color: '#aaa' }}>No Wix services found yet. Run Sync Wix Bookings to fetch the current Wix service list.</p>
          ) : (
            <div style={{ display: 'grid', gap: '10px' }}>
              {wixServicesForMapping.map((service) => {
                const serviceName = service.wix_service_name
                ...
                return (
                  <div key={service.wix_service_id || normalizeWixServiceKey(serviceName)} style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1.2fr) repeat(5, minmax(130px, 1fr)) minmax(130px, auto)', gap: '8px', alignItems: 'end', padding: '10px', border: `1px solid ${needsMapping ? 'rgba(255,204,102,0.55)' : '#2f2a20'}`, background: needsMapping ? '#1c170c' : '#0b0b0b', overflowX: 'auto' }}>
                    <div>
                      ...
                      <br />
                      <span style={{ color: '#777', fontSize: '11px', overflowWrap: 'anywhere' }}>{service.wix_service_id || 'Legacy service without Wix ID'}</span>
                      <br />
                      <span style={{ color: needsMapping ? '#ffcc66' : '#8fbf9b' }}>{needsMapping ? 'Needs mapping' : 'Mapped'}</span>
                    </div>
                    <label style={{ display: 'grid', gap: '4px' }}>
                      Glow service type
                      <select value={draft.service_type} onChange={(e) => updateWixMappingDraft(service, 'service_type', e.target.value)} style={{ padding: '8px' }}>
                    ...
                    <label style={{ display: 'grid', gap: '4px' }}>
                      Bed
                      <select value={draft.bed_id} disabled={draft.service_type !== 'sunbed'} onChange={(e) => updateWixMappingDraft(service, 'bed_id', e.target.value)} style={{ padding: '8px' }}>
                        {beds.map((bed) => <option key={bed.id} value={bed.id}>{getBedName(bed.id)}</option>)}
                      </select>
                    </label>
                    <label style={{ display: 'grid', gap: '4px' }}>
                      Minutes / duration
                      <select value={draft.minutes} disabled={draft.service_type === 'ignore'} onChange={(e) => updateWixMappingDraft(service, 'minutes', e.target.value)} style={{ padding: '8px' }}>
                        {COMMON_BOOKING_MINUTES.map((minute) => <option key={minute} value={minute}>{minute}</option>)}
                      </select>
                    </label>
                    <label style={{ display: 'grid', gap: '4px' }}>
                      Spray tan type
                      <select value={draft.service_type === 'patch_test' ? 'Patch Test' : draft.spraytan_service} disabled={!['spraytan', 'patch_test'].includes(draft.service_type)} onChange={(e) => updateWixMappingDraft(service, 'spraytan_service', e.target.value)} style={{ padding: '8px' }}>
                        {SPRAY_TAN_SERVICES.map((service) => <option key={service.name} value={service.name}>{service.name}</option>)}
                      </select>
                    </label>
                    <label style={{ display: 'grid', gap: '4px' }}>
                      Default status
                      <select value={draft.default_status} disabled={draft.service_type === 'ignore'} onChange={(e) => updateWixMappingDraft(service, 'default_status', e.target.value)} style={{ padding: '8px' }}>
      ...
      <div style={{ background: 'linear-gradient(180deg, #fff9ed, #f2eadb)', border: '1px solid rgba(122, 94, 45, 0.22)', borderRadius: '12px', padding: '12px', color: '#1c1710' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '12px' }}>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {showManagerView && <button type="button" onClick={exportStaffCalendarPdf}>Export Staff Calendar PDF</button>}
          </div>
        </div>
                    ...
                    <div key={`${member.id}-${date}`} style={{ background: '#fffdf8', border: '1px solid rgba(122, 94, 45, 0.16)', borderRadius: '10px', padding: '8px', minHeight: '74px', display: 'flex', flexDirection: 'column', justifyContent: dayEntries.length === 0 ? 'center' : 'flex-start' }}>
                      ...
                      <button
                        ...
                        onClick={(event) => {
                          ...
                          startStaffScheduleEntryForCell(member, date)
                        }}
